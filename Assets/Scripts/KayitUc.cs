...
public class KayitUc : MonoBehaviour {
    ...
    void Start () {
        ...
        kayitDepo = GameObject.FindGameObjectWithTag("KayitDepo2");
        nameOfPlayer = kayitDepo.GetComponent<KayitIki>().GetNameofPlayer();
        // jokerNerdeKullandi = kayitDepo.GetComponent<Kayit>().GetJokerNerdeKullanildi();
        jokerUsed = kayitDepo.GetComponent<KayitIki>().jokerUsed;
        if (jokerUsed == false)
        {
        ...
        sorular = System.IO.File.ReadAllLines(@"C:\udemy oyun\Soru-Cevap 2019\Assets\sorular-cevaplar\sorularSezon3.txt");
        cevaplar = System.IO.File.ReadAllLines(@"C:\udemy oyun\Soru-Cevap 2019\Assets\sorular-cevaplar\cevaplarSezon3.txt");
    ...
    {
        jokerNerdeKullandi = (kacinciSoru-1).ToString() + ". soruda kullandığınız joker gelecek sezonlarda başınızı ağrıtabilir. Ağrıtacak...";
    }

    public string RaporVer()
    {
        return "İkinci sezonu da atlattınız. " + jokerNerdeKullandi;
    }
